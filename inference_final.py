...
# -*- coding: utf-8 -*-
"""
Inference‑time mask‑alignment augmentation with optional CAMA shift.
-------------------------------------------------------------------
* If CAMA is enabled:
    – When best_x, best_y falls on the current input mask → keep mask as‑is
...
# --------------------------------------------------------------------------------
# 예시 실행 커맨드는 원본 주석 그대로 두었습니다.
# --------------------------------------------------------------------------------
# CUDA_VISIBLE_DEVICES=0 python inference_noise_mask_align_seperate_no_overlap_dynamic_random_mask_0.33_no_mask_filtering_best_point_overlap.py --model_ckpt_root='../dreambooth_ours/model/dreambooth_batch_4_5000_noise_0.0_0.33' --categorys metal_nut --text_noise_scale 0.0 --defect_json ./datasets/defect_classification_cable_include_no_toothbrush.json --anomaly_strength_min 0.0 --anomaly_strength_max 0.6 --match_json ./datasets/left_right_obj_argmax_matching_point.json --normal_masks ./normal_masks_auto_sam --output_name ./auto_sam_flip_no_flitering_no_black_mask_flitering_final_match_no_overleap_two_point_line --mask_dir ./Aug_mask_0.33_shot_flip_no_flitering --anomaly_stop_step 20 --CAMA 

###############################################################################
# 1) parse_args & common helpers
###############################################################################
def parse_args():
    parser = argparse.ArgumentParser(description="Inference‑time mask‑alignment augmentation.")  # noqa: E501
    parser.add_argument("--defect_json", type=str, required=True,
                        help="Path to JSON containing class_val (0 / 1) per defect class.")  # noqa: E501
    parser.add_argument("--match_json", type=str, required=True,
                        help="Path to JSON containing best matching pixels.")  # noqa: E501
    parser.add_argument("--model_ckpt_root", type=str, required=True,
                        help="Root directory of DreamBooth checkpoints.")
    parser.add_argument("--categorys", type=str, nargs='+', default=None,
    ...
    parser.add_argument("--text_noise_scale", type=float, default=0.0,
                        help="Std‑dev of Gaussian noise added to prompt embeddings (0 = none).")  # noqa: E501
    ...
    parser.add_argument("--anomaly_stop_step", type=int, default=999999,
                        help="Apply anomaly_strength only for the first <N> steps.")  # noqa: E501
    parser.add_argument("--normal_masks", type=str, default="./normal_masks",
                        help="Folder containing object masks for normal images.")
    parser.add_argument("--mask_dir", type=str, default="./Aug_mask_3_shot",
                        help="Directory that stores pre‑generated binary masks.")
    parser.add_argument("--CAMA", action="store_true",
                        help="Enable mask‑alignment (CAMA). Otherwise use the mask as‑is.")  # noqa: E501
    parser.add_argument("--use_random_mask", action="store_true",
                        help="Let the pipeline draw a random convex hole in the mask.")  # noqa: E501
...
def CAMA(
    ...
    the global target (best_x, best_y).  **New rule**:
      • If (best_x, best_y) already lies ON the input mask → no shift.
      • Else                                             → shift as before.
    ...
    best_x_480, best_y_480 = cand[0]  # exactly one entry
    ...
    if 0 <= best_x < W and 0 <= best_y < H and code_mask_bin[best_y, best_x] > 0:
        # 그대로 사용 (class_val==0이면 obj_mask와 AND)
